...
def page1(c,D):
    ...
    checkbox(c,M,y-0.5,checked=True); mtext(c,['2.1. Сопровождаемый','багаж, включая','ручную кладь'],M+4.5,y,7)
    checkbox(c,M+50,y-0.5,checked=False); mtext(c,['2.2. Несопровождаемый багаж','(багаж, следующий отдельно от','въезжающего/выезжающего лица)'],M+54.5,y,7)
    checkbox(c,M+120,y-0.5,checked=False); mtext(c,['2.3. Доставляемые в адрес','(пересылаемые) товары','без въезда/выезда лица'],M+124.5,y,7)
    y-=14
    t(c,'3. Сведения о товарах:',M,y,8,NB);y-=6
    selected=D.get('selected_types',[])
    imp2=D.get('direction','import')=='import'; checkbox(c,M,y,checked=imp2);t(c,'Ввоз (свободное',M+4.5,y,7);t(c,'обращение)',M+4.5,y-3.8,7)
    checkbox(c,M+45,y,checked=not imp2);t(c,'Вывоз',M+49.5,y,7)
    checkbox(c,M+75,y,checked=False);t(c,'Временный вывоз',M+79.5,y,7)
    checkbox(c,M+125,y,checked=False);t(c,'Транзит (для товаров,',M+129.5,y,7);t(c,'ввозимых со льготой)',M+129.5,y-3.8,7)
    y-=10
    items=[
