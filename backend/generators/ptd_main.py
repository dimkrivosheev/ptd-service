...
def mtext(c,lines_list,x,y,sz=6.5,leading=3.5):
    for ln in lines_list: t(c,ln,x,y,sz);y-=leading
...
def fline(c,x,y,w,label='',value='',vsz=8,lsz=5,bold=False):
    line(c,x,y,x+w,y)
...
def page1(c, D):
    ...
    mtext(c,['Заполняется физическим лицом, достигшим 16-летнего возраста.',
        ...
        'Экземпляр оформленной ПТД сохраняется на весь период временного пребывания на таможенной',
        'территории ЕАЭС (за её пределами) и предъявляется таможенному органу при въезде (возвращении).'],M,y,6,3.3)
    y-=15
    ...
    y-=9; fline(c,M,y,RW,'(адрес временного проживания (пребывания) в государстве — члене ЕАЭС — заполняется иностранным лицом)',D.get('address_temp',''),bold=bool(D.get('address_temp','')))
    y-=8; hw=RW/2-2
    ...
    y-=9
    ch=D.get('children_count','0')
    t(c,f'Со мной следуют лица, не достигшие 16-летнего возраста, в количестве ',M,y,7)
    t(c,str(ch) if ch else '0',M+115,y,7,NB); t(c,' чел.',M+121,y,7)
    ...
    bt=D.get('baggage_type','')
    checkbox(c,M,y-0.5,checked=(bt=='accompanied')); mtext(c,['2.1. Сопровождаемый','багаж, включая','ручную кладь'],M+4.5,y,7)
    ...
    # Раздел 3 — направление
    t(c,'3. Сведения о товарах:',M,y,8,NB); y-=6
    dr=D.get('direction','')
    checkbox(c,M,y,checked=(dr=='import')); t(c,'Ввоз (свободное',M+4.5,y,7); t(c,'обращение)',M+4.5,y-3.8,7)
    checkbox(c,M+45,y,checked=(dr=='export')); t(c,'Вывоз',M+49.5,y,7)
    checkbox(c,M+75,y,checked=(dr=='temp_export')); t(c,'Временный вывоз',M+79.5,y,7)
    checkbox(c,M+125,y,checked=(dr=='transit')); t(c,'Транзит (для товаров,',M+129.5,y,7); t(c,'ввозимых со льготой)',M+129.5,y-3.8,7)
    y-=10
    ...
    items=[
        ...
        ('3.2.',['Товары, ввозимые с освобождением от уплаты таможенных пошлин, налогов (ввоз со льготой)'],'льгота' in selected),
        ...
        ('3.8.',['Коллекционные материалы по минералогии, палеонтологии, кости ископаемых животных'],'minerals' in selected),
        ('3.9.',['Образцы биологических материалов человека'],'bio' in selected),
        ('3.10.',['Другие товары, в отношении которых подлежат соблюдению запреты и ограничения','и требуется представление подтверждающих документов и (или) сведений'],'other' in selected),
    ]
...
def page2(c, D):
    ...
    cols_w=[8,100,36,36]
    hdrs=[['№','п/п'],
        ['Наименование товара и его описание (идентификационный номер','и другие сведения, документы, подтверждающих соблюдение условий','ввоза с освобождением от уплаты таможенных пошлин, налогов, и (или)','документа, подтверждающего соблюдение ограничений, а также','наименования органов, выдавших такие документы)'],
        ['Вес (кг)/','количество','(л, шт.)'],
        ['Стоимость','(валюта','ЕАЭС,','евро или','доллары','США)']]
    th=22; cx=M
    for w,hd in zip(cols_w,hdrs):
        rect(c,cx,y-th,w,th); ly=y-3
        for ln in hd: t(c,ln,cx+0.8,ly,4.8,NB); ly-=3
        cx+=w
    y-=th

    # Собираем все строки раздела 4
    rows4=[]
    for item in D.get('lgota_items',[]):
    ...
    for item in D.get('animals_items',[]):
        desc='; '.join(filter(None,[item.get('type',''),item.get('species',''),f"чип:{item.get('microchip','')}" if item.get('microchip') else '',item.get('doc_vet','')]))
        rows4.append((desc,str(item.get('count','')),''))
    for key in ['minerals_items','bio_items','other_items']:
        for item in D.get(key,[]):
            desc='; '.join(filter(None,[item.get('name',''),item.get('description',''),item.get('doc_name',''),item.get('doc_num','')]))
            rows4.append((desc,f"{item.get('weight','')} {item.get('unit','')}".strip(),f"{item.get('value','')} {item.get('currency','')}".strip()))
    ...
    y-=10
    checkbox(c,M,y,checked=has_v and vt=='auto'); t(c,'Авто- и мототранспортное средство',M+4.5,y,7)
    checkbox(c,M+80,y,checked=has_v and vt=='trailer'); t(c,'Прицеп',M+84.5,y,7)
    y-=8

    if vt in ('auto','trailer'):
        a,b,cv,d2=58,52,42,18
        ...
        fline(c,M+a+b+cv+6,y,d2,'(рабочий объём (см³))',D.get('cc',''),bold=True)
        y-=10
        e,f,g,h=55,40,38,38
        fline(c,M,y,e,'(номер кузова)',D.get('body_num',''),bold=True)
        fline(c,M+e+2,y,f,'(номер шасси)',D.get('chassis_num','') if D.get('chassis_num','') else '',bold=bool(D.get('chassis_num','')))
    ...
    # Водное судно
    t(c,'Водное судно',M,y,7)
    line(c,M+18,y,M+55,y); t(c,'(вид)',M+18,y-3.5,5,color=colors.HexColor('#555'))
    line(c,M+57,y,M+110,y); t(c,'(рег. номер, страна)',M+57,y-3.5,5,color=colors.HexColor('#555'))
    if vt=='water':
        t(c,D.get('vessel_type',''),M+18,y+1.5,6.5,NB)
        t(c,D.get('reg_number',''),M+57,y+1.5,6.5,NB)
    line(c,M+112,y,M+140,y); t(c,'(стоимость)',M+112,y-3.5,5,color=colors.HexColor('#555'))
    if vt=='water': t(c,D.get('price_str',''),M+112,y+1.5,6.5,NB)
    line(c,M+142,y,M+160,y); t(c,'(масса, кг)',M+142,y-3.5,5,color=colors.HexColor('#555'))
    if vt=='water': t(c,D.get('vessel_mass',''),M+142,y+1.5,6.5,NB)
    line(c,M+162,y,M+180,y); t(c,'(длина, м)',M+162,y-3.5,5,color=colors.HexColor('#555'))
    if vt=='water': t(c,D.get('vessel_length',''),M+162,y+1.5,6.5,NB)
    y-=10

    # Воздушное судно
    t(c,'Воздушное судно',M,y,7)
    line(c,M+22,y,M+58,y); t(c,'(вид)',M+22,y-3.5,5,color=colors.HexColor('#555'))
    line(c,M+60,y,M+118,y); t(c,'(рег. номер, страна)',M+60,y-3.5,5,color=colors.HexColor('#555'))
    if vt=='air':
        t(c,D.get('aircraft_type',''),M+22,y+1.5,6.5,NB)
        t(c,D.get('reg_number',''),M+60,y+1.5,6.5,NB)
    line(c,M+120,y,M+150,y); t(c,'(стоимость)',M+120,y-3.5,5,color=colors.HexColor('#555'))
    if vt=='air': t(c,D.get('price_str',''),M+120,y+1.5,6.5,NB)
    line(c,M+152,y,M+180,y); t(c,'(масса пустого, кг)',M+152,y-3.5,5,color=colors.HexColor('#555'))
    if vt=='air': t(c,D.get('aircraft_mass',''),M+152,y+1.5,6.5,NB)
    y-=10

    # Замененная часть
    t(c,'Часть транспортного средства, замененная в государстве, не являющемся членом ЕАЭС, и подлежащая учету (регистрации)',M,y,7)
    y-=5
    rp='; '.join(filter(None,[D.get('replaced_part_name',''), D.get('replaced_part_num','')]))
    line(c,M+28,y,M+180,y); t(c,'(наименование, номер)',M+29,y-3.5,5,color=colors.HexColor('#555'))
    if rp: t(c,rp,M+29,y+1.5,6.5,NB)
    y-=10
    ...
    fline(c,M,y,28,'(дата)',D.get('sign_date',''),bold=True); fline(c,M+30,y,28,'(подпись)',''); fline(c,M+65,y,110,'(Ф.И.О. лица, действующего от имени и по поручению декларанта, реквизиты документа)','')
...
def page_annex(c, D):
    ...
    bd=D.get('birth_date','')
    if bd:
        parts=bd.split('-'); bd=f"{parts[2]}.{parts[1]}.{parts[0]}" if len(parts)==3 else bd
    ...
    # Страница 4 — источник, цель, маршрут
    y=275
    ...
    transport=cash.get('transport',''); tx=M
